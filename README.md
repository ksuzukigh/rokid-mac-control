...
- USBとWi-Fiの両方で接続できる
- 「Photo to Mac」での撮影後にライブ映像へ自動で戻る
...
カメラを使わず、黒い背景にRokidの文字やアイコンを表示します。
...
- **Rokidの開発用5ピンケーブル**
  RV101付属の3ピンケーブルは充電専用で、Macとの接続には使えません。開発用ケーブルはRokidの販売元またはサポートへ確認してください。
- **MacとRokidを接続するWi-Fi**
...
7. Rokid側にUSB接続の確認が表示された場合は許可します。
8. MacにRokidの画面が表示されたら接続完了です。
...
`A`でアプリ一覧を開くと、そこから矢印キーと`Enter`が使えるようになります。アプリを開いて`Esc`で一覧へ戻ったあとも、そのまま矢印で選び直せます。矢印キーが使える状態は、`H`・`M`・マウス操作のいずれかで終わります。

Rokidの画面の上には、いま使えるキーが常に表示されます。ふだんは`M メモ　H Home　A アプリ`、アプリ一覧を選んでいる間は`← → 選択　Enter 決定　Esc 戻る`に切り替わります。
...
2. Rokidで「Wi-Fi ON」を開き、「Wi-Fiはオンです」と表示されるまで待ちます。
3. `Rokid Control`を一度終了して開き直します。
4. それでも接続できない場合は、開発用5ピンケーブルでMacとRokidをつなぎます。
...
3. 矢印キーと`Enter`は、`A`でアプリ一覧を開いてから使えます。画面上の案内が`← → 選択`に変わっているか確認します。`H`・`M`・マウスのクリックを行うと通常の案内へ戻り、矢印キーは効かなくなります。
...
2. Rokidでカメラを使っている別のアプリを終了し、`Rokid Control`を開き直します。
