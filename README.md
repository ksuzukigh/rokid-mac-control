...
| `↑` | 下段から上段へ戻る |
| `Enter` | 決定・起動 |
| `Esc` | 一つ前に戻る（下段の選択中は選択解除） |
| `H` | ホーム画面に戻り、中央のHomeを開く |
| `Space` | 画面中央をタップ |
| 素早く`Space` 2回 | 画面中央をダブルタップ |

ホーム画面で`↓`を押すと、下段中央のHomeに水色の丸い印が表示されます。`←` / `→`で「メモ」「Home」「アプリ一覧」を選び、`Enter`で開きます。`↑`または`Esc`で上段へ戻ります。

ホーム以外の画面では、上下左右キーを通常の方向キーとして送ります。

<details>
<summary>1.1.0から更新した方へ</summary>

従来の`Shift` + `←` / `→`で下段左右を直接開く操作は廃止しました。現在は`↓`で下段へ移動し、`←` / `→`で選び、`Enter`で開きます。
...
3. `↓`で下段へ移動する操作は、Rokidのホーム画面でのみ使えます。
