...
>
> 従来の`Shift` + `←` / `→`で下段左右を直接開く操作は廃止しました。現在は`↓`で下段へ移動し、`←` / `→`でメモ・Home・アプリ一覧を順番に選べます。選択中の項目は、Mac上のRokid画面に表示される青い印で確認できます。
...
ホーム画面で`↓`を押すと、Macに表示されたRokid画面上で、下段中央のHomeに選択状態を示す青い印が表示されます。`←` / `→`で「メモ」「Home」「アプリ一覧」を選び、`Enter`で開きます。`↑`または`Esc`で上段へ戻ります。ホーム以外の画面では、上下左右キーを通常の方向キーとして送ります。
