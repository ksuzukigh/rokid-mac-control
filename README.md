...
現在の正式版は**1.2.0**です。更新内容は[変更履歴](CHANGELOG.md)で確認できます。
...
>
> 従来の`Shift` + `←` / `→`で下段左右を直接開く操作は廃止しました。現在は`↓`で下段へ移動し、`←` / `→`でメモ・Home・アプリ一覧を順番に選べます。選択中の項目は、Mac上のRokid画面に表示される水色の丸い印で確認できます。
...
ホーム画面で`↓`を押すと、Macに表示されたRokid画面上で、下段中央のHomeに選択状態を示す水色の丸い印が表示されます。`←` / `→`で「メモ」「Home」「アプリ一覧」を選び、`Enter`で開きます。`↑`または`Esc`で上段へ戻ります。ホーム以外の画面では、上下左右キーを通常の方向キーとして送ります。
...
5. それでも接続できない場合は、`~/Library/Logs/Rokid Control.log`を開き、`接続先確認`で始まる行を確認してください。不具合報告の際はこの行を添えてください。

## キーが動かないとき

1. Macに表示されているRokidのウインドウ内を1回クリックします。
2. 改善しない場合は、Macの「システム設定」→「プライバシーとセキュリティ」→「アクセシビリティ」を開き、`Rokid Control`がオンになっているか確認します。
3. `↓`を押しても下段に移動しないときは、Rokidのホーム画面が表示されているか確認します。ホーム画面以外では、上下左右キーは通常の方向キーとして送られます。
...
Macを再起動すると、Rokid Controlが使っていた常駐プロセスも終了します。
